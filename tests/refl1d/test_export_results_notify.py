...
def test_bundle_failure_surfaces_and_csv_still_written(tmp_path, monkeypatch):
    """A failure in the standard bumps bundle must be surfaced AND not block the CSV.

    bumps' ``export_fit`` writes the regular files early then runs unguarded
    uncertainty plots; on a big simultaneous DREAM fit that tail can raise after
    the regular files landed. The CSV is independent, so it must still be written,
    and the bundle failure must reach the user instead of silently vanishing.
    """
    notifications = _patch_state_and_notifications(monkeypatch, _make_problem())

    def boom_bundle(*args, **kwargs):
        raise RuntimeError("uncertainty-plot blow-up")

    monkeypatch.setattr(api, "export_fit", boom_bundle)

    asyncio.run(api.export_results([str(tmp_path)]))

    # CSV salvaged despite the bundle failure ...
    assert list(tmp_path.glob("*-pars.csv")), "CSV should be written even if the bundle fails"
    # ... and the bundle failure is shown, carrying the real reason.
    bundle_notes = [n for n in notifications if "incomplete" in n["title"].lower()]
    assert bundle_notes, [n["title"] for n in notifications]
    assert "uncertainty-plot blow-up" in bundle_notes[0]["content"]
